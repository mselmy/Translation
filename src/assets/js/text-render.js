...
function textRender() {
  ...
  document.getElementById("englishText").outerHTML =
    '<div class="form-control" id="englishText" style="white-space: pre-line;">' +
    englishText +
    "</div>";
...
document.addEventListener("click", function (e) {
  if (e.target.id === "englishText" && e.target.tagName === "DIV") {
    document.getElementById("englishText").outerHTML =
      '<textarea class="form-control" id="englishText" rows="16" placeholder="Enter text here">' +
      unrenderEnglishText +
      "</textarea>";
  } else if (e.target.id === "arabicText" && e.target.tagName === "DIV") {
    document.getElementById("arabicText").outerHTML =
      '<textarea class="form-control" id="arabicText" rows="16" placeholder="Enter text here" style="direction: rtl;">' +
      unrenderArabicText +
      "</textarea>";
  }
});
